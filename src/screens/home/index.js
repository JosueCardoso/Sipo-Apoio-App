...
const index = ({ navigation }) => {
    ...
    for (let i = 0; i < MenuFactory.length; i++) {
        if (i % 2 == 0) {
            elementsRight.push(<Icon key={MenuFactory[i].id} text={translate(MenuFactory[i].text)} icon={MenuFactory[i].icon} onPress={() => getOnPress(MenuFactory[i])} />)
        } else {
            elementsLeft.push(<Icon key={MenuFactory[i].id} text={translate(MenuFactory[i].text)} icon={MenuFactory[i].icon} onPress={() => getOnPress(MenuFactory[i])} />)
        }
    }
